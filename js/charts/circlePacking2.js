function renderCirclePacking2(container, datasets) {
  const MIN_CHARS = 3;

  const rootSel = d3.select(container);
  const svg = rootSel.select('#circlepacking2-svg');
  let yearSelect = rootSel.select('#circlepacking2-year');
  const sharedSelect = d3.select('#shared-year-select');
  if (!sharedSelect.empty()) {
    yearSelect = sharedSelect;
  }
  const tooltip = ensureTooltip();

  const data = normalizeData(datasets.civilianFatalities || []).filter(d => d.country);
  ...
  function ensureTooltip() {
    ...
    if (!t.empty()) {
      ...
      t = d3.select('body').append('div').attr('id', 'circlepacking2-tooltip');
    }
    return t
      .attr('role', 'tooltip')
  ...
  function normalizeData(arr) {
    const norm = d => ({
      ...
      fatalities: +(d.FATALITIES || d.Fatalities || d.fatalities || d.CIVILIAN_FATALITIES || d.civilianFatalities || d.EVENTS || d.Events || d.events || 0)
    });
  ...
  function populateYearSelect(sel, years) {
    if (sel.empty()) return;
    // Guard: don't re-populate if already done (shared select scenario)
    if (sel.attr('data-populated') === '1') return;
  ...
  yearSelect.on('change.circlePacking2', update);
  update();
}
